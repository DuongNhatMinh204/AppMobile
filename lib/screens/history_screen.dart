...
class HistoryScreen extends StatelessWidget {
  final FirebaseFirestore _firestore = FirebaseFirestore.instance;
  final AuthService authService = AuthService(); // Tạo AuthService

  Future<List<Map<String, dynamic>>> _fetchUserBookings() async {
    String? currentUserPhone = await authService.getCurrentUserPhone(); // Lấy số điện thoại user

    if (currentUserPhone == null) {
      return []; // Nếu chưa đăng nhập, không có lịch sử
    }

    QuerySnapshot querySnapshot = await _firestore
        .collection("bookings")
        .where("userId", isEqualTo: currentUserPhone) // Chỉ lấy của user hiện tại
        .get();
    ...
    return Scaffold(
      ...
      body: FutureBuilder<List<Map<String, dynamic>>>(
        ...
        builder: (context, snapshot) {
          ...
          if (!snapshot.hasData || snapshot.data!.isEmpty) {
            return Center(child: Text("Bạn chưa đặt vé nào."));
          }
